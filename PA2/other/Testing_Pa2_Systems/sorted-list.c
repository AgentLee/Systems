...
* Authors: Niharika Nagar () and Jon Lee (jl1424)
*
*/
/*
* Big-O Analysis:
*/
/*Need:
* - Special Case: what happens when a sorted list is modified when
...
SortedListPtr SLCreate(CompareFuncT cf, DestructFuncT df){
	...
	SortedListPtr nullPtr;
	if(cf == NULL || df == NULL) return nullPtr;
	*/
...
int SLInsert(SortedListPtr list, void* newObj){
	...
	while(ptr != NULL){
		...
		/*printf("%d, %d, %d\n", compare, *(int*)ptr->value, *(int*)newObj);*/
		if(compare < 0){
...
/*keep refcount the same for removed items*/
...
void * SLNextItem(SortedListIteratorPtr iter){
	...
	/*if(iter->list->numItems > 0){
		...
		/*check if removed before iterating to next*/
		temp = iter->curr;
		iter->curr = iter->curr->next;
		iter->prev = temp;
